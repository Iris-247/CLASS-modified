...
1.屏蔽了最初使用的玻尔兹曼模式
2.初始条件绝热ur，
3.没有伪压强，没有高阶近似拟合项，既然不存在剪切演化
4.ca2采用最简单的经验公式，ceff2很低，得到行为类似于冷暗物质
5.同样，试图通过其他手段和精度控制修复guage artifact的问题，失败还原，发现这个度规问题似乎不止存在于扰动环节，调整精度是一项大工程？。
5.由于嫁接在原有玻尔兹曼初始化上，导致artifact在小尺度，不仅在演化早期十分明显，晚期也十分显著，这是某种自激发的误差机制？但只要将补偿上牛顿度规，物理行为类冷暗物质。
6.同样，推测是由于浮点精度问题我没有进行微调，导致生长出非物理的一阶项和常数项；或者就是我经验有限，没有察觉到一些隐性条件？*//*输入"标"检索修改位置*/
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_cdm",((pba->has_cdm == _TRUE_)&& (ppt->gauge != synchronous)));/*标*/
...
int perturbations_solve(
  ...
  if (pba->has_ncdm == _TRUE_) {
    for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_ncdm1+n_ncdm]/ppw->pvecback[pba->index_bg_rho_ncdm1+n_ncdm]-1./3.)>1,/*标*/
                 ppt->error_message,
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      idx = ppw->pv->index_pt_psi0_ncdm1;
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          ppw->pv->y[idx]  = ppw->pv->y[ppw->pv->index_pt_delta_cdm]*(1+w_fixed);           // Psi_0 
  
          ppw->pv->y[idx+1] = 0;        // Psi_1 
      ...
      if (pba->has_ncdm == _TRUE_) {
        ...
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++) {
            ...
            ppw->pv->y[idx]  = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g];                
      ...
      */
      ...
      if (pba->has_ncdm == _TRUE_) {
        ...
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
            ...
            ppw->pv->y[idx]  -= 3.*a_prime_over_a*alpha;                
  
            ppw->pv->y[idx+1] = k*k*alpha;   
    ...
    //       ppw->pv->y[idx+1] = ppw->pv->y[ppw->pv->index_pt_theta_cdm];        // Psi_1 
    ...
    if ((pba->has_ncdm == _TRUE_) || (pba->has_cdm == _TRUE_)){
      ...
      numerator += (4.0/3.0) * ppw->pvecback[pba->index_bg_rho_g]
      * ppw->pv->y[ppw->pv->index_pt_theta_g];

      numerator += ppw->pvecback[pba->index_bg_rho_b] 
      * ppw->pv->y[ppw->pv->index_pt_theta_b];

      numerator += (ppw->pvecback[pba->index_bg_rho_ncdm1] 
      + ppw->pvecback[pba->index_bg_p_ncdm1]) 
      * ppw->pv->y[ppw->pv->index_pt_psi0_ncdm1+1];
      
      ppw->pv->y[ppw->pv->index_pt_theta_cdm] = 
      -numerator / ppw->pvecback[pba->index_bg_rho_cdm];
...
/*标*/
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppr->ncdm_fluid_approximation != ncdmfa_none) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
        ...
        for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
          ...
          pseudo_p_ncdm = pvecback[pba->index_bg_pseudo_p_ncdm1+n_ncdm]; 
          pseudo_p_ncdm = 0 ;/* pseudo-pressure (see CLASS IV paper) */
          w_ncdm = p_ncdm_bg/rho_ncdm_bg; /* equation of state parameter */
          ca2_ncdm =  w_ncdm;/* adiabatic sound speed */
          ...
          //dy[idx] = -(1 + w_ncdm)*metric_continuity
          //- 3*a_prime_over_a*(ceff2_ncdm - ca2_ncdm)
          //*(y[idx]+3.0*a_prime_over_a*(1 + w_ncdm)*y[idx+1]/k2);
          ...
          /** - -----> different ansatz for approximate shear derivative *//*标志*/
