...
int background_functions(
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    for (n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++) {
      ...
      double w_fixed = 1e-6;/*标*/
      double rho_ncdm = pba->Omega0_ncdm[n_ncdm] * pow(pba->H0, 2) * pow(a, -3.0 * (1.0 + w_fixed));
      double p_ncdm = w_fixed * rho_ncdm;
      double pseudo_p_ncdm = 0;  // 保持结构一致
      ...
      rho_tot += rho_ncdm;
      p_tot   += p_ncdm;
      dp_dloga += -(2. + 3.*w_fixed)* p_ncdm;  

      // 分别记入辐射与物质的ρ
      rho_r += 3.0 * p_ncdm;
      rho_m += rho_ncdm - 3.0 * p_ncdm;
    }
  }
